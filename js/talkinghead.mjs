...
class TalkingHead {
  ...
  */
  constructor(node, opt = null ) {
    ...
    this.isGestureSequencePaused = false;
    this.currentGestureTimeout = null;
    this.remainingGestures = [];
  ...
  */
  ...
  // Update the playGestureSequence method
  playGestureSequence(gestureList, onComplete) {
    if (!Array.isArray(gestureList) || gestureList.length === 0) {
      console.error("Invalid gesture list provided");
      if (onComplete) onComplete();
      return;
    }

    this.remainingGestures = [...gestureList];
    this.isGestureSequencePaused = false;

    const playNextGesture = () => {
      if (this.isGestureSequencePaused) return;

      if (this.remainingGestures.length === 0) {
        if (onComplete) onComplete();
        return;
      }

      const currentGesture = this.remainingGestures.shift();
      const { name, duration = 3, mirror = false, transitionTime = 1000 } = currentGesture;

      this.playGesture(name, duration, mirror, transitionTime);

      // Schedule the next gesture
      this.currentGestureTimeout = setTimeout(playNextGesture, duration * 1000 + transitionTime);
    };
  ...
  // Add these new methods to the TalkingHead class

  pauseGestureSequence() {
    this.isGestureSequencePaused = true;
  ...
  resumeGestureSequence() {
    if (!this.isGestureSequencePaused) return;

    this.isGestureSequencePaused = false;
    this.playGestureSequence(this.remainingGestures);
  }

  isGestureSequencePaused() {
    return this.isGestureSequencePaused;
  }

  
  async startSpeakingWithoutAudio(force = false) {
    if (!this.armature || (this.isSpeaking && !force) || this.isPaused) return;
    this.stateName = 'talking';
    this.isSpeaking = true;
    if (this.speechQueue.length) {
      let line = this.speechQueue.shift();
      if (line.emoji) {
        ...
        let duration = line.emoji.dt.reduce((a, b) => a + b, 0);
        this.animQueue.push(this.animFactory(line.emoji));
        setTimeout(this.startSpeakingWithoutAudio.bind(this), duration, true);
      } else if (line.break) {
        // Break
        setTimeout(this.startSpeakingWithoutAudio.bind(this), line.break, true);
      } else if (line.anim) {
        // Look at the camera
        this.lookAtCamera(500);
        this.speakWithHands();
  
        // Reset lips and set mood
        this.onSubtitles = line.onSubtitles || null;
        this.resetLips();
        if (line.mood) this.setMood(line.mood);
  
        // Add animations to the queue
        const startTime = this.animClock;
        line.anim.forEach((x) => {
          const adjustedAnim = {
            ...x,
            ts: x.ts.map(t => startTime + t)
          };
          this.animQueue.push(adjustedAnim);
        });
  
        // Use the duration provided by speakWithoutAudio
        const totalDuration = line.duration || 0;
        
        this.speakingTimeout = setTimeout(() => {
          if (!this.isPaused) {
            this.startSpeakingWithoutAudio(true);
          }
        }, totalDuration);
      } else if (line.marker) {
        if (typeof line.marker === "function") {
  ...
  speakWithoutAudio(r, opt = null, onsubtitles = null) {
    opt = opt || {};
    const lipsyncLang = opt.lipsyncLang || this.avatar.lipsyncLang || this.opt.lipsyncLang;
    const o = {};
    ...
    if (r.words) {
      ...
      for (let i = 0; i < r.words.length; i++) {
        ...
        if (word.length) {
          ...
          // Calculate visemes based on the word
          const w = this.lipsyncPreProcessText(word, lipsyncLang);
          const v = this.lipsyncWordsToVisemes(w, lipsyncLang);
          if (v && v.visemes && v.visemes.length) {
            const wordDuration = duration;
            const visemeDuration = wordDuration / v.visemes.length;
            
            for (let j = 0; j < v.visemes.length; j++) {
              const t = startTime + j * visemeDuration;
              lipsyncAnim.push({
                template: { name: 'viseme' },
                ts: [t, t + visemeDuration * 0.5, t + visemeDuration],
                vs: {
                  ['viseme_' + v.visemes[j]]: [null, (v.visemes[j] === 'PP' || v.visemes[j] === 'FF') ? 0.9 : 0.6, 0]
                }
              });
    ...
    if (Object.keys(o).length) {
      this.speechQueue.push(o);
      this.speechQueue.push({ break: 300 });
      this.startSpeakingWithoutAudio();
    }
  }

  pauseSpeakingWithoutAudio() {
    if (this.isSpeaking && !this.isPaused) {
      this.isPaused = true;
      this.isSpeaking = false;
      this.pausedTime = this.animClock;
      
      // Pause all ongoing animations
      this.animQueue.forEach(anim => {
        if (anim.ts) {
          anim.pausedTs = anim.ts.map(t => t - this.pausedTime);
        }
      });
      ...
      console.log("Speaking paused at:", this.pausedTime);
    }
  }

  resumeSpeakingWithoutAudio() {
    if (this.isPaused) {
      this.isPaused = false;
      const resumeTime = this.animClock;
      const timeDiff = resumeTime - this.pausedTime;

      // Resume all paused animations
      this.animQueue.forEach(anim => {
        if (anim.pausedTs) {
          anim.ts = anim.pausedTs.map(t => t + resumeTime);
          delete anim.pausedTs;
        }
      });

      this.stateName = 'talking';
      console.log("Speaking resumed at:", resumeTime, "Time difference:", timeDiff);
  ...
  pauseAll() {
    if (!this.isPaused) {
      ...
      // Pause gesture animations
      if (this.currentGestureTimeout) {
        clearTimeout(this.currentGestureTimeout);
      }

      // Pause speaking
      this.pauseSpeakingWithoutAudio();

      console.log("All animations paused at:", this.pausedTime);
    }
  }
  ...
  resumeAll() {
    if (this.isPaused) {
      ...
      // Resume gesture animations
      if (this.remainingGestures && this.remainingGestures.length > 0) {
        this.playGestureSequence(this.remainingGestures);
      }

      // Resume speaking
      this.resumeSpeakingWithoutAudio();

      console.log("All animations resumed at:", resumeTime, "Time difference:", timeDiff);
    }
  }

  stopSpeakWithoutAudio() {
    // Clear the speech queue
    this.speechQueue.length = 0;
    
    // Clear any ongoing animations, including visemes
    this.animQueue = [];
    ...
    // Reset lips and render
    if (this.armature) {
      this.resetLips();
      this.render();
    }
    
    console.log("Speaking without audio stopped");
  }
